...
import 'schema/quizes_record.dart';
...
export 'schema/quizes_record.dart';
...
Future<int> queryUserAccountRecordCount({
...
Stream<List<UserAccountRecord>> queryUserAccountRecord({
  ...
  bool singleRecord = false,
    ...
    queryCollection(
      UserAccountRecord.collection,
      UserAccountRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
...
/// Functions to query QuizesRecords (as a Stream and as a Future).
Future<int> queryQuizesRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      QuizesRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<QuizesRecord>> queryQuizesRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      QuizesRecord.collection,
      QuizesRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<QuizesRecord>> queryQuizesRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  final userRecord = UserAccountRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await UserAccountRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createUserAccountRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      UserAccountRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createUserAccountRecordData(email: email));
}
